...
class Range extends Period
{
    ...
    protected function generate()
    {
        ...
        } elseif ( $this->strPeriod == 'hour' && $hourRange = Range::parseHourRange($this->strDate)){
            /**
             * [Thangnt 2016-09-20]
             * Create Range for Hour
             * 22:24 Range is now compatible with Hour. That means system now accept the multi Hour periods.
             */
            $strDateStart = $hourRange[1];
            $strDateEnd = $hourRange[2];
            $startDate = Date::factory($strDateStart);
     ...
     * @return mixed  array(1 => dateStartString, 2 => dateEndString) or `false` if the input was not a date range.
     */
    public static function parseHourRange($dateString)
    {
        $regs = null;
        if(preg_match('/^(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})$/D', trim($dateString), $regs)) {
            return false;
        } elseif (preg_match('/^([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})$/D', trim($dateString), $regs)) {
            $regs[] = $regs[1]." 23:00:00";
            $regs[1] = $regs[1]." 00:00:00";
            return $regs;
        } elseif (preg_match('/^([0-9]{4}-[0-9]{1,2}-[0-9]{1,2}),([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})$/D', trim($dateString), $regs)) {
            $regs[1] = $regs[1]." 00:00:00";
