...
class GetVideoTitles extends Base
{
    ...
    public function configureView(ViewDataTable $view)
    {
        ...
        $view->config->filters[] = function (DataTable $table) use ($view) {
            ...
            if (!empty($a) || !empty($b)) {
                $view->config->show_footer_message = Piwik::translate('MediaAnalytics_UnknownMediaTitleExplanation', array($unknown, '<a target="_blank" rel="noreferrer" href="https://developer.piwik.org/guides/media-analytics/options">', '</a>'));
            }
        };
    }
