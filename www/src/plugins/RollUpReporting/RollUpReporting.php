...
class RollUpReporting extends Plugin
{
    ...
    public function addIdSitesToLiveQueries(&$idSites)
    {
        ...
        if ($this->isRollUpIdSite($idSite)) {
            $model = $this->getModel();
            $childIdSites = $model->getChildIdSites($idSite);
    ...
    public function addIdSitesToBeArchivedAtOnce(&$idSites, $period)
    {
        ...
        if ($cache->contains($cacheKey)) {
            ...
            $idSitesOfRollUpSite = $this->getModel()->getChildIdSites($idSite);
            // we cannot add the rollUpIdSite, otherwise the archiver runs in an endless loop.
            $cache->save($cacheKey, $idSitesOfRollUpSite);
        }
    ...
    {
        $model = $this->getModel();
        $rollUpIdSites = $model->getParentIdSites();
        ...
        $idSites = array_merge($idSitesToBeArchivedWithoutRollUpIdSites, $rollUpIdSitesToBeArchived);
    }

    public function addIdSitesNotUsingTracker(&$idSites)
    {
        $model = $this->getModel();
        $rollUpSites = $model->getParentIdSites();

        if (!empty($rollUpSites)) {
            $idSites = array_merge($idSites, $rollUpSites);
        }
    }
